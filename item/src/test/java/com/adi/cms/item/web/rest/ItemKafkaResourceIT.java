...
import com.adi.cms.item.config.KafkaProperties;
...
class ItemKafkaResourceIT {
    ...
    void setup() {
        ...
        ItemKafkaResource kafkaResource = new ItemKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/item-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/item-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
