...
import com.adi.cms.character.config.KafkaProperties;
...
class CharacterKafkaResourceIT {
    ...
    void setup() {
        ...
        CharacterKafkaResource kafkaResource = new CharacterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/character-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/character-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
