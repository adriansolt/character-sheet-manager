...
import com.adi.cms.xaracter.config.KafkaProperties;
...
class XaracterKafkaResourceIT {
    ...
    void setup() {
        ...
        XaracterKafkaResource kafkaResource = new XaracterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xaracter-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xaracter-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
