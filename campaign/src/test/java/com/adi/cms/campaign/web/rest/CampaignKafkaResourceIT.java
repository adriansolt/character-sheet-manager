...
import com.adi.cms.campaign.config.KafkaProperties;
...
class CampaignKafkaResourceIT {
    ...
    void setup() {
        ...
        CampaignKafkaResource kafkaResource = new CampaignKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/campaign-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/campaign-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
